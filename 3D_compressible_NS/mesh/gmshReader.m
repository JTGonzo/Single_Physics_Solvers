...
file = 'RB_sq_v4_temp.msh';
wrkDir = '../geometry/' ;
problemString = 'RB' ;
elemType = '8Hex' ;

% Read mesh file and associate physical properties
fileId = fopen(file);
while ~feof(fileId)
    ...
    elseif strcmp(tmp,'$Elements');
        ...
        for i=1:nEl
            ...
            elseif (Elem.Type(i) == 15)
                conn1Point(i,:) = fscanf(fileId,'%d',[1 1]);
            end
        end
...
% Write connectivity files for the element types
...
% Save element, coordinate and node data to .mat files
BcNodes1 = [];
BcNodes2 = [];
BcNodes3 = [];
BcNodes4 = [];
...
for i=1:nEl
    ...
    elseif (Elem.Type(i) == 5) %8Hex
        if (Elem.Tag(i,1) == pnId2(1))
            BcNodes1 = [BcNodes1; conn8Hex(i,:)];
            nBC1 = size(unique(BcNodes1),1) ;
        elseif (Elem.Tag(i,1) == pnId2(2))
            BcNodes2 = [BcNodes2; conn8Hex(i,:)];
            nBC2 = size(unique(BcNodes2),1) ;
        elseif (Elem.Tag(i,1) == pnId2(3))
            BcNodes3 = [BcNodes3; conn8Hex(i,:)];
            nBC3 = size(unique(BcNodes3),1) ;
        elseif (Elem.Tag(i,1) == pnId2(4))
            BcNodes4 = [BcNodes4; conn8Hex(i,:)];
            nBC4 = size(unique(BcNodes4),1) ;
        elseif (Elem.Tag(i,1) == pnId2(5))
            BcNodes5 = [BcNodes5; conn8Hex(i,:)];
            nBC5 = size(unique(BcNodes5),1) ;
        elseif (Elem.Tag(i,1) == pnId2(6))
            BcNodes6 = [BcNodes6; conn8Hex(i,:)];
            nBC6 = size(unique(BcNodes6),1) ;
        elseif (Elem.Tag(i,1) == pnId2(7))
            BcNodes7 = [BcNodes7; conn8Hex(i,:)];
            nBC7 = size(unique(BcNodes7),1) ;
        elseif (Elem.Tag(i,1) == pnId2(8))
            BcNodes8 = [BcNodes8; conn8Hex(i,:)];
            nBC8 = size(unique(BcNodes8),1) ;
        elseif (Elem.Tag(i,1) == pnId2(9))
            BcNodes9 = [BcNodes9; conn8Hex(i,:)];
            nBC9 = size(unique(BcNodes9),1) ;
        elseif (Elem.Tag(i,1) == pnId2(10))
            BcNodes10 = [BcNodes10; conn8Hex(i,:)];
            nBC10 = size(unique(BcNodes10),1) ;
        end		
    end
end
...
save('BCAll.mat','BCAll');
save('BCLeft.mat','BCLeft');
save('BCRight.mat','BCRight');
save('BCTop.mat','BCTop');
save('BCBottom.mat','BCBottom');
save('BCFront.mat','BCFront');
save('BCBack.mat','BCBack');
save('conn.mat','conn');
