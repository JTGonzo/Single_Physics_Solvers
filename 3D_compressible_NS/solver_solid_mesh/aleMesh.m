...
function [Sol] = aleMesh(Sol, solver, solid, BCCylinder, BCTop, BCBottom,...
                          BCLeft, BCRight, pmc, cnnF, cnnS, crd, elemType, nen,...
                          ndof, nElem, BCStructure, crdNew)
...
for i=1:nen
   xxf(:,i) = crd(cnn(:,i),2);
   yyf(:,i) = crd(cnn(:,i),3);
   xxfNew(:,i) = crdNew(cnn(:,i),2);
   yyfNew(:,i) = crdNew(cnn(:,i),3);
...
for p = 1:nQuad  
    ...
    if ~isempty(negJacobian)
       printf('Mesh deformed, Negative Jacobian');
    end
    volume = abs(volume);
    
    tau = (max(volume)-min(volume))./volume ;
...
freeNodes = unique([BCBound; unique(BCCylinder(:)); unique(BCStructure(:))]');
